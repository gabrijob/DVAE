...
def build_cSRNN(cfg, device='cpu'):

    ### Load parameters for cSRNN
    # General
    ...
    # Build model
    model = cSRNN(x_dim=x_dim, c_dim=c_dim, z_dim=z_dim, activation=activation,
                 dense_x_h=dense_x_h,
...
class cSRNN(nn.Module):

    def __init__(self, c_dim, x_dim, z_dim=16, activation = 'tanh',
    ...
    def build(self):
        ...
        if len(self.dense_x_h) == 0:
            dim_x_h = self.x_dim + self.c_dim
            dic_layers['Identity'] = nn.Identity()
        else:
            dim_x_h = self.dense_x_h[-1]
            for n in range(len(self.dense_x_h)):
                if n == 0:
                    dic_layers['linear'+str(n)] = nn.Linear(self.x_dim + self.c_dim, self.dense_x_h[n])
                else:
        ...
        if len(self.dense_hx_g) == 0:
            dim_hx_g = self.dim_RNN_h + self.x_dim + self.c_dim
            dic_layers['Identity'] = nn.Identity()
        else:
            dim_hx_g = self.dense_hx_g[-1]
            for n in range(len(self.dense_hx_g)):
                if n == 0:
                    dic_layers['linear'+str(n)] = nn.Linear(self.dim_RNN_h+self.x_dim+self.c_dim, self.dense_hx_g[n])
                else:
        ...
        if len(self.dense_hz_x) == 0:
            dim_hz_x = self.dim_RNN_h + self.z_dim + self.c_dim
            dic_layers['Identity'] = nn.Identity()
        else:
            dim_hz_x = self.dense_hz_x[-1]
            for n in range(len(self.dense_hz_x)):
                if n == 0:
                    dic_layers['linear'+str(n)] = nn.Linear(self.dim_RNN_h+self.z_dim+self.c_dim, self.dense_hz_x[n])
                else:
    ...
    def generation_x(self, z, c, h):
        # 1. z_t and c_t to zc_t
        zc_t = torch.cat((z,c), -1)

        # 2. zc_t and h_t to y_t
        hz_x = torch.cat((h, zc_t), -1)
    ...
    def forward(self, x_c):
        ...
        _, batch_size, xc_dim = x_c.shape
        x = x_c[:, :, :self.x_dim]
        c = x_c[:, :, -self.c_dim:]

        # main part
        xc_0 = torch.zeros(1, batch_size, xc_dim).to(self.device)
        xc_tm1_only = torch.cat((x[:-1, :, :], c[:-1, :, :]), -1)
        xc_tm1 = torch.cat((xc_0, xc_tm1_only), 0)
        #x_0 = torch.zeros(1, batch_size, self.x_dim).to(self.device)
        #x_tm1 = torch.cat((x_0, x[:-1, :, :]), 0)
        #c_0 = torch.zeros(1, batch_size, self.c_dim).to(self.device)
        #c_tm1 = torch.cat((c_0, c[:-1, :, :]), 0)
        #xc_tm1 = torch.cat((x_tm1, c_tm1), -1)
        h = self.deterministic_h(xc_tm1)

        self.z, self.z_mean, self.z_logvar = self.inference(x_c, h)
        ...
        y, self.y_lower_bound, self.y_upper_bound = self.generation_x(self.z, c, h)
...
if __name__ == '__main__':
    ...
    z_dim = 16
    device = 'cpu'
    csrnn = cSRNN(x_dim=x_dim, z_dim=z_dim).to(device)
    model_info = csrnn.get_info()
    for i in model_info:
        print(i)

    x = torch.ones((2,513,3))
    y, z_mean, z_logvar, z_mean_p, z_logvar_p, z = csrnn.forward(x)
